...
bool MotorCaptureControl::updateThermalProtection(Context &ctx)
{
#ifdef FEATURE_NTC
...
void MotorCaptureControl::resetFlowController()
{
...
#if MOTOR_FLOW_CONTROL_MODE == MOTOR_FLOW_CONTROL_CLOSED_LOOP
void MotorCaptureControl::updateClosedLoopMotors(Context &ctx)
{
...
void MotorCaptureControl::updateMotors(Context &ctx)
{
    if (motorOverheatActive)
    {
        ctx.components.motor.setPwmImmediate(0);

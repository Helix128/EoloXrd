...
void LogService::logTaskWorker(void *arg)
{
...
bool LogService::initSD(Context &ctx)
{
    if (sdInitAttempted)
        return isSdReady;
...
void LogService::markSdFailed()
{
    sdStatus = SD_ERROR;
    isSdReady = false;
}

void LogService::startLogTask(Context &ctx)
{
    if (logQueue == nullptr)
...
void LogService::enqueueLogData(Context &ctx)
{
    if (logQueue == nullptr)
        startLogTask(ctx);
...
bool LogService::logsIdle() const
{
    return !logActive && (logQueue == nullptr || uxQueueMessagesWaiting(logQueue) == 0);
}

void LogService::processCaptureSample(Context &ctx)
{
    ctx.saveSession();
    logData(ctx);
#ifdef FEATURE_MODEM
...
bool LogService::logData(Context &ctx)
{

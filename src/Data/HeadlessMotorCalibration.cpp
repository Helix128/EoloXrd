...
void HeadlessMotorCalibration::resetRun()
{
    _pointCount = 0;
    _currentPwm = 0;
    _phaseStartMs = 0;
...
bool HeadlessMotorCalibration::start(const HeadlessMotorCalibrationConfig &config)
{
...
void HeadlessMotorCalibration::setState(State state)
{
    _state = state;
    _phaseStartMs = millis();
}

bool HeadlessMotorCalibration::isRunning() const
{
...
bool HeadlessMotorCalibration::isFinished() const
{
    return _state == State::Done || _state == State::Error || _state == State::Aborted;
}

const char *HeadlessMotorCalibration::stateText() const
{
    switch (_state)
    {
...
float HeadlessMotorCalibration::minFlow() const
{
    return _pointCount > 0 ? _points[0].flow : 0.0f;
}

float HeadlessMotorCalibration::maxFlow() const
{
    return _pointCount > 0 ? _points[_pointCount - 1].flow : 0.0f;
}

void HeadlessMotorCalibration::fail(Context &ctx, const char *error)
{
    _error = error;
...
void HeadlessMotorCalibration::abort(Context &ctx)
{
    if (!isRunning())
        return;
    ctx.components.motor.setPwmImmediate(0);
...
void HeadlessMotorCalibration::clear(Context &ctx)
{
    resetRun();
    setState(State::Idle);
...
bool HeadlessMotorCalibration::save(Context &ctx)
{
...
bool HeadlessMotorCalibration::storeCurrentPoint()
{
    if (_sampleCount == 0)
        return false;
...
bool HeadlessMotorCalibration::validateAndApply(Context &ctx)
{
...
void HeadlessMotorCalibration::update(Context &ctx)
{
    if (!isRunning())
        return;
